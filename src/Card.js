...
const Card = async (data, site, theme, layout) => {
  const thumbnailBase64 = await imgToDataURL(data.profile_image);
  const siteImageBase64 = `data:image/png;base64,${images[site]}`;
  const namePosition = (210 - (data.display_name.length * 8.5)) / 2;
  const isDark = theme === 'dark';

  if (layout === 'compact') {
    return `
      <svg width="191" height="70" viewBox="0 0 191 70" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
        <g fill="none" fill-rule="evenodd">
          ...
          <image x="135" y="20" width="50" height="50" xlink:href="${siteImageBase64}"/>
          </g>
      </svg>
    `;
  }
